...
`InputBar` 组件套件（`input-bar.client.spec.tsx`）原样通过，`ui-conversation` 全量套件（27 个文件、425 个用例）全绿——新的 `openPath` prop、backdrop 分支与点击处理器无回归。
