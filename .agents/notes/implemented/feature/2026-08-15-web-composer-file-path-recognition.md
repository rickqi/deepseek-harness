...
The `InputBar` component suite (`input-bar.client.spec.tsx`) continues to pass unchanged, and the full `ui-conversation` suite (27 files, 425 cases) is green, so the new `openPath` prop, the backdrop branch, and the click handler introduce no regression.
