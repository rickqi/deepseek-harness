...
export function InputBar({
  ...
  const backdrop: ReactNode[] = []
  {
    ...
    // offset, every text-ref range, and every path range — merged in draft
    // order. The sources never overlap by construction: structured references
    // own their ranges, a same-start text-ref that is a strict prefix of a
    // path ref is filtered at derivation (the path is the more specific
    // reading), and the claim token only leads.
    let cursor = 0
