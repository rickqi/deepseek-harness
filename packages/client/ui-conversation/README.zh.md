...
草稿的文件路径引用是装饰产物：`deriveDecorations` 用 `scanPathRefs` 扫描草稿（纯文本，不做文件系统解析），`DraftDecorations.pathRefs` 承载这些范围，因此路径形状的 token——绝对 `/a/b`、`./`/`../`/`~/` 相对、UNC `\\server\share\…`、以及至少两段的盘符 `X:\…`/`X:/…` 形式——以独立色调加虚线下划线渲染在 textarea 自身的字形上。Ctrl/⌘+点击高亮范围内部即通过注入的 `openPath` 回调打开该路径，`apply.ts` 把它接到助手文件提及打开所用的同一个 `workspaces.openPath`，token 在交给 host 前先按会话 cwd 解析；打不开的路径静默失败、绝不变成 composer 错误，无会话时该手势无效（[决定](../../../.agents/notes/implemented/feature/2026-08-15-web-composer-file-path-recognition.md)）。
